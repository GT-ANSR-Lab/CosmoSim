...
def enqueue_command(
    ...
    """Add a flow-generation command while validating dependencies."""
    ...
    if not graph_dir.exists():
        raise FileNotFoundError(
            ...
            "Run `graph_generation/generate_digraphs.py` before generating directed flows."
        )
    ...
    command = (
        ...
        f"python -m spectrum_management.utils.generate_flows_digraph "
        f"{shlex.quote(str(DIGRAPH_DATA_DIR))} {shlex.quote(str(graph_dir))} "
        f"{shlex.quote(str(constellation_dir))} {start_time} {beam_allocation} {ku_band_capacity} "
