...
def enqueue_command(
    ...
    """Add a capacity-generation command while validating dependencies."""
    ...
    if not graph_dir.exists():
        raise FileNotFoundError(
            ...
            "Run `graph_generation/generate_digraphs.py` before generating directed capacities."
        )

    log_file = (
        command_logs_dir
        / (
            f"capacities_{constellation_config_id}_for_{start_time}_"
            f"{beam_allocation}_{routing_policy}_{ku_band_capacity}.log"
        )
    )
    ...
    command = (
        ...
        f"{routing_policy} {beam_allocation} {ku_band_capacity} "
        ...
        for country in countries:
            for pop in pop_values[country]:
                for ut_dist in ut_distribution[1:]:
                    for ku_band_capacity in ku_band_capacities[:2]:
                        constellation_config_id = f"{constellation}_{isl}_ground_stations_starlink_cells_{country}_0_{pop}_{ut_dist}_{ku_band_capacity}"
...
for constellation in constellations[:1]:
    for isl in isls[:1]:
        for country in countries:
            for pop in pop_values[country]:
                for ut_dist in ut_distribution[:2]:
                    for ku_band_capacity in ku_band_capacities[:1]:
                        for pop_limit in pop_limits[country]:
                            constellation_config_id = f"{constellation}_{isl}_ground_stations_starlink_cells_{country}_0_{pop}_{ut_dist}_{pop_limit}_{ku_band_capacity}"
                            update_interval_ms = 1000
                            duration_s = 15
                            print(constellation_config_id)
                            graph_path = f"{constellation}_{isl}_ground_stations_starlink_cells_{country}_0"
...
ut_distribution = ["waterfill", "population"]
...
# commands_to_run = commands_to_run[:2]
# Run the commands
