...
def generate_cell_priority(all_cells, user_terminals):
    cell_terminals = {}
    for cell in all_cells:
        cell_terminals[cell] = 0
    
    for ut in user_terminals:
        cell_terminals[ut["cell_id"]] += 1
    
    
    max_terminals = max(cell_terminals.values())
    # min_terminals = min(cell_terminals.values())
    interval = max_terminals / 8
    print(cell_terminals, max_terminals, interval)
    cell_priority = {}
    
    for cell in all_cells:
        cell_priority[cell] = round(cell_terminals[cell] / interval)
    
    print(cell_priority)
    return cell_priority

def priority_beam_mapping(all_cells, satellites, satellite_cells, cell_satellites, user_terminals, sat_capacity):
    cell_sat_mapping = {}
    beams_assigned = 0
    ...
    cell_priority = generate_cell_priority(all_cells, user_terminals)
    ...
    #     print("Satellite iteration", sat, len(cells), cells, cell_priority)

    #     sat_beams_assigned = 0

    #     while sat_beams_assigned < 8 * frequency_reuse_factor:
    #         weights = np.array([cell_priority[cell] for cell in cells])
    #         if np.sum(weights) == 0:
    #             break
    #         cell_for_allocation = str(np.random.choice(cells, p=weights/np.sum(weights)))
    #         print("Cell for allocation", cell_for_allocation, cells)
            
    #         assigned = False
    #         for ch in range(8):
    #             dummy_node = cell_for_allocation + "_" + str(ch)
    #             if assigned:
    #                 break
    #             if dummy_node not in cell_sat_mapping:
    #                 for freq in range(frequency_reuse_factor):
    #                     sat_beam = str(freq) + "_" + str(sat) + "_" + str(ch)
    #                     if sat_beam in all_sat_beams and check_compatibility(cell_for_allocation, cell_sat_mapping, sat_beam):
    #                         cell_sat_mapping[dummy_node] = sat_beam
    #                         beams_assigned += 1
    #                         cell_priority[cell_for_allocation] = max(0, cell_priority[cell_for_allocation] - 1)
    #                         cell_satellite_assignments[cell_for_allocation].append(sat)
    #                         cell_channels_assigned[cell_for_allocation].append(ch)
    #                         all_sat_beams.remove(sat_beam)
    #                         sat_beams_assigned += 1
    #                         assigned = True
    #                         print("mapping", dummy_node, sat_beam)
    #                         break

    #         # not possible to assign a beam to this cell
    #         if not assigned:
    #             cells.remove(cell_for_allocation)
