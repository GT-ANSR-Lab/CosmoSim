...
def enqueue_command(
    ...
    """Add a capacity-mask command while validating dependencies."""
    ...
    if not graph_dir.exists():
        raise FileNotFoundError(
            ...
            "Run `graph_generation/generate_digraphs.py` before applying masks."
        )

    log_file = (
        command_logs_dir
        / f"masks_{constellation_config_id}_for_{start_time}_{beam_allocation}_{mask}.log"
    )

    command = (
        f"cd {shlex.quote(str(PROJECT_ROOT))}; "
        f"python -m traffic_engineering.utils.generate_capacities_masks "
        ...
        f"{mask} {beam_allocation} > {shlex.quote(str(log_file))} 2>&1"
    )
