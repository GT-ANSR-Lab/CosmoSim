...
def distance_m_ground_station_to_cell(cell_id, satellite, epoch_str, date_str):
    """
    Computes the straight distance between a cell and a satellite in meters

    :param cell_id:         Cell Id
    :param satellite:       The satellite
    :param epoch_str:       Epoch time of the observer (cell) (string)
    :param date_str:        The time instant when the distance should be measured (string)
    ...
    observer.epoch = epoch_str
    observer.date = date_str
    observer.lat = str(lat)   # Very important: string argument is in degrees.
    observer.lon = str(lon)  # DO NOT pass a float as it is interpreted as radians
    observer.elevation = 0
...
def satellite_in_ut_range(ut, satellite, epoch_str, time_str, bounding_dist_m):
    distance_m = distance_m_ground_station_to_satellite(ut, satellite, epoch_str, time_str)
    return distance_m <= bounding_dist_m
