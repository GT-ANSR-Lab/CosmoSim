#!/usr/bin/env python3
"""Compare CosmoSim beam allocation strategies."""
...
def read_data(country: str, ku_band_capacity: float, routing: str, flow_time: int) -> np.ndarray:
...
def plot(country: str, ku_band_capacity: float, routing: str, flow_time: int, output_dir: Path) -> None:
    pop_values, data = read_data(country, ku_band_capacity, routing, flow_time)
    output_dir.mkdir(parents=True, exist_ok=True)
    plt.figure(figsize=(6.4, 3.2))
    for beam_policy, series in zip(BEAM_POLICIES, data):
        averages = [np.mean(samples) / 1000 for samples in series]
        label = "Coordinated" if beam_policy == "greedy-coordinated" else "Uncoordinated"
        linestyle = "--" if beam_policy == "greedy-coordinated" else "-"
    ...
    base = output_dir / f"beam_allocation_{country}_ku{ku_band_capacity:g}"
    plt.savefig(base.with_suffix(".pdf"))
    plt.savefig(base.with_suffix(".png"), dpi=300)
    plt.close()

def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot impact of beam allocation policies on capacity.")
