#!/usr/bin/env python3
"""Batch runner for workflows/generate_capacities.py using existing demand outputs."""
...
WORKFLOW = PROJECT_ROOT / "workflows" / "generate_capacities.py"
DATA_ROOT = PROJECT_ROOT / "data"
DEMANDS_ROOT = DATA_ROOT
LOG_ROOT = DATA_ROOT / "command_logs"
RUN_LOG_ROOT = LOG_ROOT / "run_generate_capacities"
...
def build_command(
    ...
    log_path = LOG_ROOT / f"{scenario_label}_t{flow_time}_{beam_policy}_{routing_policy}.log"
    ...
    description = f"{scenario_label} (t={flow_time}, beam={beam_policy}, route={routing_policy})"
...
def enqueue_cases() -> List[CommandSpec]:
            ...
            for country in COUNTRIES:
                ...
                for pop in pop_list:
                    for dist_config in DISTRIBUTIONS:
                        caps = (
                            GCB_CAPS.get(country, [])
                            if dist_config.requires_gcb_cap
                            else [None]
                        )
                        for gcb_cap in caps:
                            for ku_band_capacity in dist_config.ku_band_capacities:
                                scenario_ku = (
                                    ku_band_capacity if dist_config.include_ku_in_label else None
                                )
                                ...
                                for beam_policy in BEAM_POLICIES:
                                    for flow_time in DEFAULT_FLOW_TIMES:
                                        demands_dir = demands_dir_for(
                                            scenario_id, flow_time, beam_policy
                                        )
                                        demands_file = demands_dir / "demands.txt"
                                        if not demands_file.exists():
                                            print(
                                                f"[skip] Missing demands at {demands_file} -- "
                                                "run run_generate_flows first"
                                            )
                                            continue
    ...
    print(f"[info] Enqueued {len(specs)} capacity jobs.")
    return specs


def run_commands(commands: List[CommandSpec]) -> None:
    if not commands:
        print("No capacity scenarios to run. Ensure demands exist or adjust the configuration.")
        return
...
def main() -> None:
    ...
    run_log_path = RUN_LOG_ROOT / f"run_generate_capacities_{timestamp}.log"
